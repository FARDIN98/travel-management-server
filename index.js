...
client.connect(err => {
  const serviceCollection = client.db("TBD").collection("service");
  const orderCollection = client.db("TBD").collection("orders");
  const reviewCollection = client.db("TBD").collection("reviews");
  const adminCollection = client.db("TBD").collection("admin");
...
app.get('/', (req, res) => {
    res.send('Hello!! Welcome to Traveliana Bangladesh')
  })
